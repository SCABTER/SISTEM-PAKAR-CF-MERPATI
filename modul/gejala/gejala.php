...
switch($_GET[act]){
  ...
  $tampil=mysqli_query($conn,"SELECT * FROM gejala ORDER BY kode_gejala");
	echo "<h6 class='m-0 font-weight-bold text-primary fas fa-radiation fa-2x text-black-50-300 '> Gejala </h6>
 
                 <form class='d-none d-sm-inline-block form-inline mr-auto ml-md-3 my-2 my-md-0 mw-100 navbar-search' method=POST action='?module=gejala' name=text_form onsubmit='return Blank_TextField_Validator_Cari()'>
                        <div class='input-group'>
                            ...
                            <div class='input-group-append'> &nbsp;
                            <button class='btn bg-olive border-1 margin btn-Primary ' type=button name=tambah value='' onclick=\"window.location.href='gejala/tambahgejala';\"><i class='fa fa-plus' aria-hidden='true'></i></button>
                        </div>
                        </div>
                    </form>     
  ...
  <form method=POST action='?module=admin' name=text_form onsubmit='return Blank_TextField_Validator_Cari()'></form>";
	...
	if ($offset!=0) {
		...
		echo "<li class='page-item '><span class=page-link  ><a href=index.php?module=gejala&offset=$prevoffset>Back</a></span></li>";
	}
	else {
	...
	for($i=1;$i<=$halaman;$i++){
		$newoffset = $limit * ($i-1);
		if($offset!=$newoffset){
			echo "<li class='page-item '><span class=page-link  ><a href=index.php?module=gejala&offset=$newoffset>$i</a></li>";
			//cetak halaman
		}
		else {
	...
	if(!(($offset/$limit)+1==$halaman) && $halaman !=1){
		...
		echo "<li class='page-item '><span class=page-link  ><a href=index.php?module=gejala&offset=$newoffset>Next</a></li>";
	}
	else {
