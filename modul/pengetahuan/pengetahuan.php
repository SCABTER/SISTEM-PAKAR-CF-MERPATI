...
switch($_GET[act]){
  ...
  $tampil=mysqli_query($conn,"SELECT * FROM basis_pengetahuan ORDER BY kode_pengetahuan");
	echo "<h6 class='m-0 font-weight-bold text-primary fas fa-sitemap fa-2x text-black-50-300'> Pengetahuan </h6>
            <form class='d-none d-sm-inline-block form-inline mr-auto ml-md-3 my-2 my-md-0 mw-100 navbar-search' method=POST action='?module=pengetahuan' name=text_form onsubmit='return Blank_TextField_Validator_Cari()'>
                        <div class='input-group'>
                            ...
                            <div class='input-group-append'> &nbsp;
                            <button class='btn bg-olive border-1 margin btn-Primary ' type=button name=tambah value='' onclick=\"window.location.href='pengetahuan/tambahpengetahuan';\"><i class='fa fa-plus' aria-hidden='true'></i></button>
                        </div>
                        </div>
                    </form>     
...
<text>
CFPAKAR = MB Pakar - MD Pakar<br>
CFUSER  = Diambil Dari Pilihan<br>
CFhe     = CFPAKAR*CFUSER
CFc (CFhe1,CFhe2) = CFhe1 + CFhe2 (1- CFhe1)                            ; jika CF1 dan CF2 keduanya posistif<br>

CFc (CF1,CF2) = CFhe1 + CFhe2 (1+ CFhe1)                            ; jika CF1 dan CF2 keduanya negative<br>

CFc (CFhe1,CFhe2) = {CFhe1 + CFhe2} / (1-min{| CFhe1|,| CFhe2|})          ; jika salah satu negatif
</text><br>";
	...
	if ($offset!=0) {
		...
		echo "<li class='page-item '><span class=page-link><a href=index.php?module=pengetahuan&offset=$prevoffset>Back</a></span></li>";
	}
	else {
	...
	for($i=1;$i<=$halaman;$i++){
		$newoffset = $limit * ($i-1);
		if($offset!=$newoffset){
			echo "<li class='page-item '><span class=page-link><a href=index.php?module=pengetahuan&offset=$newoffset>$i</a></li>";
			//cetak halaman
		}
		else {
			echo "<li class='page-item disabled '><span class=page-link><span class=current>".$i."</span></li>";//cetak halaman tanpa link
		}
	}
	...
	if(!(($offset/$limit)+1==$halaman) && $halaman !=1){
		...
		echo "<li class='page-item '><span class=page-link><a href=index.php?module=pengetahuan&offset=$newoffset>Next</a></li>";
	}
	else {
